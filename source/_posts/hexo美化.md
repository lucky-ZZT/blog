...
YAML# aplayer音乐
- <div class="aplayer no-destroy" data-id="8152976493" data-server="netease" data-type="playlist" 
```
...
```
BUSHnpm install --global gulp-cli # 全局安装gulp命令集
...
创建在 Hexo 的根目录，创建一个`sw-template.js`文件,打开`[Blogroot]/sw-template.js`,输入以下内容：
...
在`[Blogroot]\themes\butterfly\layout\includes\third-party\`目录下新建`pwanotice.pug`文件，
打开`[Blogroot]\themes\butterfly\layout\includes\third-party\pwanotice.pug`,输入：

```
JAVASCRIPT#app-refresh.app-refresh(style='position: fixed;top: -2.2rem;left: 0;right: 0;z-index: 99999;padding: 0 1rem;font-size: 15px;height: 2.2rem;transition: all 0.3s ease;')
...
修改`[Blogroot]\themes\butterfly\layout\includes\additional-js.pug`,在文件底部添加以下内容，注意缩进。`butterfly_v3.6.0`取消了缓存配置，转为完全默认，需要将`{cache:theme.fragment_cache}`改为`{cache: true}`:

```
DIFF      if theme.pjax.enable
...
新建文件名为`manifest.json`并将其放到`[Blogroot]/source`目录下，此时还不能直接用，需要添加一些内容，以下是我的`manifest.json`配置内容，权且作为参考，其中的`theme_color`建议用取色器取设计的图标的主色调，**同时务必配置 start_url 和 name 的配置项，这关系到你之后能否看到浏览器的应用安装按钮**。：
...
在`_config.butterfly.yml`中搜索 Inject 添加以下代码，将自定义 css`custom.css`引入
...
新建`[Blogroot]/source/js/ali_font.js`, 输入以下内容(在alifont中自己导出即可)
...
```
BASH# https://www.npmjs.com/package/hexo-butterfly-tag-plugins-plus
...
参考：[wowjs 特效](https://akilar.top/posts/abab51cf/)
...
参考https://anzhiy.cn/posts/asdx.html
